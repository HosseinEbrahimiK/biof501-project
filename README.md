...
In this project, we designed a pipeline to analyze a microarray gene expression data of AML disease to identify genes in AML patients that are significantly up-regulated or down-regulated (differentially expressed) compared to genes in normal cells. We used ``limma`` package available in R to perform the analysis. First, we created a design matrix indicating which sample belongs to which groups in our study, Leukemia or normal. Then, after fitting a linear model to the data with ``lmFit()`` function, we measure how much contrasts do we have between these two groups with ``makeContrasts()`` and ``contrasts.fit()``. Finally based on the contrasts matrix we calculate p-values and detect differentially expressed genes. By identitying these genes in AML patients, we would be able to study the pathways that these genes are involved, and more broadly genetic and environmental factors affecting these genes and their relationships to the disease.
...
4. Differential expression analysis and identitying up or down-regulated genes by using ``limma`` package in R
...
cd biof501-project
```
Make sure to delete files in results and data folders to enable the workflow reproduce them again.
...
```
**Important Note:** During running this command, the pipeline asks you to select a gene to plot the violin plot. You are able to see a list of up-regulated and down-regulated genes in the output ``.txt`` files and choose your gene of interest if it exists in differentially expressed genes.
